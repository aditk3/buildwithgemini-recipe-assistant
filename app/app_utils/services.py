    ...
    if agent_engine_id := os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID"):
        try:
            ...
            return VertexAiSessionService(
                project="qwiklabs-gcp-03-5bec0b757af4",
...
def get_memory_service():
    ...
    if uri := os.environ.get("MEMORY_SERVICE_URI"):
        if uri.startswith("agentengine://") and "/" not in uri.removeprefix("agentengine://"):
            engine_id = uri.removeprefix("agentengine://")
            uri = f"agentengine://projects/qwiklabs-gcp-03-5bec0b757af4/locations/us-east4/reasoningEngines/{engine_id}"
        return create_memory_service_from_options(
            base_dir=_AGENT_DIR, memory_service_uri=uri
        )
    if agent_engine_id := os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID") or "8747472618027745280":
        try:
